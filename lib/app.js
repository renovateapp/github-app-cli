...
async function getInstallations(appToken) {
  ...
  try {
    const url = process.env.RENOVATE_ENDPOINT + '/app/installations';
    const options = {
      json: true,
      headers: {
...
async function getInstallationRepositories(userToken) {
  ...
  try {
    const url =
      process.env.RENOVATE_ENDPOINT + '/installation/repositories?per_page=100';
    const options = {
      json: true,
      headers: {
        accept: 'application/vnd.github.machine-man-preview+json',
